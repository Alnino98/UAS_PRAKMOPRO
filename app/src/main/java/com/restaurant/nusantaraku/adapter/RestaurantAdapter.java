package com.restaurant.nusantaraku.adapter;
...
import com.restaurant.nusantaraku.R;
...
public class RestaurantAdapter extends RecyclerView.Adapter<RestaurantAdapter.MyViewHolder> {
    ...
    public RestaurantAdapter(Context ctx) {
        this.ctx = ctx; listItem = new ArrayList<>();
    }
    ...
    public void add(RestaurantItem item) {
        listItem.add(item); notifyItemInserted(listItem.size() + 1);
    }
    ...
    {
        listItem.clear(); notifyDataSetChanged();
    }
    public void remove(int pos) {
        listItem.remove(pos); notifyDataSetChanged();
    }
    public void swap(List<RestaurantItem> datas) {
        if (datas == null || datas.size() == 0) listItem.clear();
        if (listItem != null && listItem.size() > 0) listItem.clear();
        listItem.addAll(datas); notifyDataSetChanged();
    }
    public RestaurantItem getItem(int pos) { return listItem.get(pos);
    }
    public void setFilter(List<RestaurantItem> list) {
        listItem = new ArrayList<>(); listItem.addAll(list); notifyDataSetChanged();
    }
